...
5. **Fetch user identity.** GET `https://api.github.com/user` with the access token → `{ id, login, name, created_at, public_repos, followers, following, type, ... }`. GET `https://api.github.com/user/emails` → `[{ email, primary, verified }, ...]`. The reputation facts (`created_at`, `public_repos`, `followers`, `following`, `type`) are stored on the `PrivateProfile` as `github` with `status = "ok"` and `checkedAt = now` on every successful sign-in (cases a and b below, and on link) — see [behaviors/private-storage.md](../behaviors/private-storage.md) and [api/moderation.md](moderation.md).
