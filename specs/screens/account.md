...
A self-service settings hub for the current user's identity, sessions, and account state. Profile fields (name, bio, slug, tags, avatar) live on the **profile edit** flow (`/members/:slug/edit`) — this screen is for *account* concerns, not *profile* concerns.

## Data Requirements

- `GET /api/auth/me` — current person + account level
- `GET /api/auth/sessions` — list of remembered sessions (see note below)
...
- Email verification status:
  ...
  - If absent: amber notice "Not verified" with no action — email-change + verification flow is not yet specified
- "Linked GitHub account" — once GitHub OAuth is specified, this card grows a row showing the linked GitHub login + "Unlink" affordance. For now, this card has no GitHub row.
...
Placeholder card with greyed-out "GitHub" and "Slack" rows and a disabled "Connect" button. The card is visible to signal direction; the linking flow itself is not yet specified.
...
**From here:** `/` (after sign out), `/members/:slug/edit` (for profile fields).
