...
/**
 * The one `AuthnContextClassRef` every assertion carries, per
 * specs/api/saml.md#authentication-statement. Fixed — never echoed back from
 * the AuthnRequest's `RequestedAuthnContext`. Matches what the legacy
 * Emergence SAML2 connector asserted (`SAML2\Constants::AC_PASSWORD`) against
 * this same Slack workspace.
 */
export const SLACK_AUTHN_CONTEXT_CLASS_REF = 'urn:oasis:names:tc:SAML:2.0:ac:classes:Password';
...
export interface BuildResponseSubstitutionsOptions {
  ...
  /**
   * Opaque per-assertion `AuthnStatement/@SessionIndex`. Generated by the
   * caller alongside the Response/Assertion IDs so all three share one id
   * source; it must differ from the assertion ID (see spec).
   */
...
export function buildResponseSubstitutions(
  ...
  return {
    ...
    // AuthnStatement (specs/api/saml.md#authentication-statement). These are
    // our own placeholders — samlify's built-in template has a bare
    // `{AuthnStatement}` slot that the default (non-custom) path blanks out,
    // so we carry the statement literally in our template and fill its
    // attributes here. AuthnInstant is the assertion issue time by design.
    AuthnInstant: now,
    SessionIndex: opts.sessionIndex,
    AuthnContextClassRef: SLACK_AUTHN_CONTEXT_CLASS_REF,
...
export function buildSlackSamlEntities(settings: SamlIdpSettings): SlackSamlEntities {
  ...
  const idp = IdentityProvider({
    ...
    loginResponseTemplate: {
      ...
      // list at IdP construction; every other placeholder is filled by our
      // customTagReplacement callback (see buildResponseSubstitutions).
      //
      // Element order inside <Assertion> follows the schema (saml-core
      // §2.3.3): Issuer, [Signature — inserted after Issuer by samlify],
      // Subject, Conditions, then statements. AuthnStatement precedes
      // AttributeStatement, mirroring the legacy connector's output.
      context:
        '<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" ID="{ID}" Version="2.0" IssueInstant="{IssueInstant}" Destination="{Destination}" InResponseTo="{InResponseTo}"><saml:Issuer>{Issuer}</saml:Issuer><samlp:Status><samlp:StatusCode Value="{StatusCode}"/></samlp:Status><saml:Assertion ID="{AssertionID}" Version="2.0" IssueInstant="{IssueInstant}"><saml:Issuer>{Issuer}</saml:Issuer><saml:Subject><saml:NameID Format="{NameIDFormat}" NameQualifier="{NameQualifier}" SPNameQualifier="{SPNameQualifier}">{NameID}</saml:NameID><saml:SubjectConfirmation Method="urn:oasis:names:tc:SAML:2.0:cm:bearer"><saml:SubjectConfirmationData NotOnOrAfter="{SubjectConfirmationDataNotOnOrAfter}" Recipient="{SubjectRecipient}" InResponseTo="{InResponseTo}"/></saml:SubjectConfirmation></saml:Subject><saml:Conditions NotBefore="{ConditionsNotBefore}" NotOnOrAfter="{ConditionsNotOnOrAfter}"><saml:AudienceRestriction><saml:Audience>{Audience}</saml:Audience></saml:AudienceRestriction></saml:Conditions><saml:AuthnStatement AuthnInstant="{AuthnInstant}" SessionIndex="{SessionIndex}"><saml:AuthnContext><saml:AuthnContextClassRef>{AuthnContextClassRef}</saml:AuthnContextClassRef></saml:AuthnContext></saml:AuthnStatement>{AttributeStatement}</saml:Assertion></samlp:Response>',
      attributes: [
        {
          name: 'User.Email',
